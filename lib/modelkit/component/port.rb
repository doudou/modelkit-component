	...
	class Port
	    ...
	    attr_reader :type
            ...
            def orocos_type_name
                Types::Registry.rtt_typename(type)
            end
	    ...
	    def initialize(task, name, type, options = Hash.new)
                ...
                if type
                    ...
                    if type.name == "/std/vector<double>"
                        Component.warn "#{type.name} is used as the port type for #{name}, logging it will not be possible"
                    end
                end
	    ...
	    dsl_attribute(:doc) { |value| value.to_s }
            ...
            def self.resolve_max_size_path(type, name)
                ...
                if !(resolved_type <= Types::ContainerType)
            ...
            def self.initialize_max_size_sample(path, sample, max_sizes)
                ...
                if !(sample_t <= Types::Type) || !sample_t.contains?(Types::ContainerType)
                    return true
                end

                path = path.dup

                if sample.kind_of?(Types::ContainerType)
                ...
                elsif sample.kind_of?(Types::ArrayType)
                    if path.empty? then path = ["[]"]
                    else path[-1] = "#{path[-1]}[]"
                    end
                    element = sample_t.deference.new
                ...
                elsif sample.kind_of?(Types::CompoundType)
                    path << nil
