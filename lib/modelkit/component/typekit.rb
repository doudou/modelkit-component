module ModelKit
    module Component
        ...
        class Typekit
            ...
            def initialize(loader, name, registry = Types::Registry.new, typelist = [], interface_typelist = [])
                @loader = loader
                @name, @registry = name, registry
                @typelist = typelist.to_set
                @interface_typelist = interface_typelist.to_set
                @opaques = Array.new
                @opaque_registry = Types::Registry.new
            ...
            def find_opaque_for_intermediate(type)
                ...
                if m_type?(type)
                    ...
                    begin
                        ...
                        else raise Types::NotFound
                        end
                    rescue Types::NotFound
            ...
            def intermediate_type_name_for(type_def)
                ...
                if type.opaque?
                    ...
                    elsif type < Types::ContainerType
                        "#{type.container_kind}<#{intermediate_type_name_for(type.deference)}>"
                    else
                        path = Types.split_typename(type.name)
                        path.map! do |p|
                            p.gsub(/[<>\[\], \/]/, '_')
                        end
			"/" + path.join("/") + "_m"
            ...
            # @raises Types::NotFound if the expected intermediate type cannot
            #   be found
            ...
            # @return [Model<Types::Type>] the type of the opaque, or the
