module ModelKit
    module Component
        ...
        class Project
            ...
            # @return [Hash<String,ModelKit::Component::TaskContext>]
            attr_reader :tasks
            # The tasks defined by this project
            # @return [Hash<String,ModelKit::Component::TaskContext>]
            attr_reader :self_tasks
            # This project's deployments
            # @return [Hash<String,ModelKit::Component::Deployment::Container>]
		...
		name
            ...
            # Returns the Node object for the default task contexts
            # superclass
	    ...
	    def task_context(name, object_class: Node, &block)
		new_task = object_class.new(self, "#{self.name}::#{name}")
		new_task.instance_eval(&block) if block_given?
		tasks[new_task.name] = new_task
                self_tasks[new_task.name] = new_task
            ...
            rescue ModelKit::TaskModelNotFound => e
	    ...
	    #
            ...
            # {Component::Deployment} instance, so see the documentation of that class
	    ...
	    def deployment(name, &block) # :yield:
                ...
                deployer = Component::Deployment.new(self, name, &block)
