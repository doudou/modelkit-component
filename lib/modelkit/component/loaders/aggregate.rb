    ...
    module Loaders
        ...
        class Aggregate < Base
            ...
            def project_model_from_name(name)
                ...
                ModelKit::Loaders.debug "Aggregate: resolving #{name} on #{loaders.map(&:to_s).join(",")}"
                loaders.each do |l|
                    begin
            ...
            def task_model_from_name(name)
                ...
                ModelKit::Loaders.debug "Aggregate: resolving task model #{name} on #{loaders.map(&:to_s).join(",")}"
                loaders.each do |l|
                    begin
                        # We assume that the sub-loaders are created with self
                        # as root loader. They will therefore register
