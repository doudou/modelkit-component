module ModelKit
	...
	class Node
	    ...
	    # The task name
	    attr_reader :name
            # The subclass of Node which should be used to define this
            # class
            attr_reader :superclass
            # A set of classes the Node has to implement as well
            ...
            # Node class. +task_context+ can either be a class name or a
            # Node instance. In both cases, it must be defined in the
            ...
            def subclasses(task_context)
                if task_context.respond_to?(:to_str)
                    if @superclass != project.default_task_superclass
                        raise ModelKit::ConfigError, "#{@name} tries to subclass #{task_context} "+
	    ...
	    # in the context of the newly created Node object.
	    #
	    # Node objects should not be created directly. You should
		...
		@attributes[name] = att = configuration_object(Attribute, name, type, default_value)
                Component.load_documentation(att, /attribute/)
                att
	    end
            ...
            def configuration_object(klass, name, type, default_value)
                ...
                if default_value
                    ...
                    begin
                        Types.from_ruby(default_value, type)
                    rescue Types::UnknownConversionRequested => e
		...
		klass.new(self, name, type, default_value)
            ...
            # can be either in Types notation (/std/string) or in C++
		...
		@properties[name] = prop = configuration_object(Property, name, type, default_value)
                Component.load_documentation(prop, /property/)
                prop
	    end
            ...
            # Declares toplevel states for this node
		...
		@operations[name] = op = Operation.new(self, name)
            ...
            def hidden_operation(name, body=nil)
                ...
                op.hidden = true
                if body
                    ModelKit.warn "body argument for hidden_operation '#{self.name}'.'#{name}' is deprecated, please set the body during generation phase"
                    ModelKit.warn "If you call this from a plugin, define the implementation within your :early_register_for_generation method"
                    op.base_body(body)
                end
                op
            end
	    ...
	    def output_port(name, type, options = Hash.new)
                ...
                Component.load_documentation(port, /output_port/)
                port
	    end
	    ...
	    def input_port(name, type, options = Hash.new)
                ...
                Component.load_documentation(port, /input_port/)
                port
            end
