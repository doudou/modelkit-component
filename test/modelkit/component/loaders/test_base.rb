...
describe ModelKit::Loaders::Base do
    attr_reader :loader

    before do
        @loader = flexmock(ModelKit::Loaders::Base.new)
  ...
  <alias name="/string" source="/std/string"/>
    ...
    describe "#deployed_task_model_from_name" do
        it "raises AmbiguousName if there are more than one deployment with a task matching the name" do
            ...
            assert_raises(ModelKit::AmbiguousName) { loader.deployed_task_model_from_name(task_name) }
        end
    end
    ...
    describe "#imported_typekits_for" do
        ...
        before do
            @registry = Types::CXXRegistry.new
        ...
        it "raises DefinitionTypekitNotFound if no loaded typekits define the type" do
            assert_raises(ModelKit::DefinitionTypekitNotFound) do
