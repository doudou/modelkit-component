...
module ModelKit::Component
    describe Loaders::Base do
        attr_reader :loader

        before do
            @loader = flexmock(Loaders::Base.new)
    ...
    <typelib>
      ...
      <alias name="/int" source="/int32_t"/>
            ...
            it "should register the type-to-typekit mapping for aliased types as well" do
                assert_equal [typekit].to_set, loader.typekits_by_type_name['/int']
            end
            it "should register all interface types, including aliases" do
                assert_equal ['/int'].to_set, loader.interface_typelist
            end
        end
        ...
        describe "#deployed_task_model_from_name" do
            it "raises AmbiguousName if there are more than one deployment with a task matching the name" do
                ...
                assert_raises(AmbiguousdeployedNodeName) { loader.deployed_task_model_from_name(task_name) }
            end
        end
        ...
        describe "#imported_typekits_for" do
            ...
            before do
                ...
                registry.create_numeric '/int32_t'
                @typekit = ModelKit::Component::Typekit.new(loader, "typekit", registry, [])
                @definition_typekit = ModelKit::Component::Typekit.new(loader, "definition_typekit", registry, ['/int32_t'])
